...
describe('NovedadFeedbackComponent (#188)', () => {
  ...
  it('una imagen de más de 2 MB se avisa sin adjuntarla', fakeAsync(() => {
    const grande = new File([new Uint8Array(2 * 1024 * 1024 + 1)], 'captura.png', { type: 'image/png' });

    component.adjuntarImagen(grande);
    tick();

    expect(component.imagenAdjunta).toBeNull();
    expect(component.errorImagen).toContain('máximo 2 MB');
  }));
